...
Hard-cut recovery is controlled by the policy via `onRecovery()`. When agents are killed by KV pressure before calling `report()`, the policy decides per-agent whether to extract. The pool forks from the agent's branch, generates with a grammar-constrained schema, and captures partial findings. Configure via `DefaultAgentPolicyOpts.recovery`.
