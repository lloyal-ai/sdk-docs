...
The class is constructed from a `SessionContext` and thresholds from the policy. It captures an immutable snapshot of KV state at construction time -- the snapshot does not update as the tick progresses.
...
const policy = new DefaultAgentPolicy({
  budget: { context: { softLimit: 2048, hardLimit: 128 } },
});
...
<Note>
`cellsUsed` increments on every `decode_each` (COMMIT) and `decode_scatter` (SETTLE prefill). It **decrements** when a branch is pruned — each branch subtracts its unique cells (`position - fork_head`) on release. It resets to zero on bulk operations like `retainOnly` and `drain`, and resets to the surviving branch's position on `retainOnly`.

This means `remaining` and `percentAvailable` reflect actual KV usage and **recover** as branches are freed. When an agent reports and its branch is pruned (`pruneOnReport: true`), the freed cells are visible to the next `ContextPressure` snapshot. Remaining agents see increased headroom.
...
```typescript
const policy = new DefaultAgentPolicy({
  budget: { context: { softLimit: 3072 } },  // reserve for inner pool
});
