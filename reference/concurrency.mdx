...
Tools can spawn sub-agents. A tool's `execute()` method returns an Effection [`Operation<unknown>`][ops], which means it can [`yield*`][ops] into any framework primitive — `createAgentPool`, `useAgentPool`, `createAgent`, `diverge`. The inner pipeline shares the same GPU compute, KV cache, and event channel as the outer pool. No separate process. No serialization boundary.
...
One toolkit at every level — composed by `createAgentPool()`:
