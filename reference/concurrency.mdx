...
description: "Four-phase tick loop, decision boundary, shared resources, and recursive agent spawning."
---
...
The decision boundary depends on the model choosing to continue exploring rather than terminating. This choice is influenced by the ordering of tools in the `createToolkit()` array — see [Grammar & Tool Ordering](/reference/grammar-and-ordering#tool-ordering-sensitivity) for the full mechanism and design rule.
...
`CallingAgent` propagates lineage. During DISPATCH, the pool sets `CallingAgent` to the agent that made the tool call. When the tool spawns an inner pool, each inner agent reads `CallingAgent` and records it as its parent. This enables `walkLineage()` — traversing from inner agent up through the calling chain to the outermost agent.

### Inner pool execution

The inner `withSharedRoot` forks from the calling agent's branch (warm path) or creates a fresh root (cold path). Inner agents inherit the calling agent's full attention state — every prior tool call, every tool result, every token generated. The inner `useAgentPool` runs the same four-phase tick loop as the outer pool. Inner agents share the same `BranchStore` and compete for the same KV cache slots.
...
Hard-cut recovery is now built into the pool via `reportPrompt`. When agents are killed by KV pressure before calling `report()`, the pool forks from their branch, generates with a grammar-constrained schema, and captures partial findings. A confabulation guard (fewer than 100 tokens or fewer than 2 tool calls) prevents extraction from agents with insufficient context.
```
...
Declarative [Tool Guards](/learn/agents#tool-guards) enforce research-before-delegation. Before dispatching `web_research` or `research`, the guard checks the agent's **local** tool history — not the full lineage. Each agent must have done at least one search AND one fetch/read before it can delegate to sub-agents.

<Note>
The local-history requirement exists because of Continuous Context. When sub-agents fork from the calling agent's branch, they inherit the parent's full KV — including all search and fetch results. A lineage-based guard would let children bypass the research requirement by counting the parent's history. This produces blind relay chains — the question passes through multiple levels verbatim with zero hypothesis refinement. The local-history guard forces each agent to do its own research, discover concepts from the source material, and only then delegate with refined sub-questions.
</Note>
