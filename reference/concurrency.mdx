...
One toolkit at every level — composed by `spawnAgents()`:

```typescript
const pool = yield* spawnAgents({
  tools: source.tools,               // search, read_file, grep
  systemPrompt: RESEARCH_PROMPT,
  tasks: questions,
  terminalTool: { name: 'report', tool: reportTool },
  recursive: { name: 'research' },   // self-referential delegation
});
```
