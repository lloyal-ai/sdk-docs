...
Outer pool:
  withSharedRoot (outer root, pos 0..866)
    ...
    │           └── withSharedRoot (inner root, fork from A0 + 2 token separator)
    │                 ├── Agent A0.0 (fork from inner root)
    │                 ├── Agent A0.1 (fork from inner root)
    │                 └── Agent A0.2 (fork from inner root)
...
When `withSharedRoot` receives a `parent` branch option, it forks from the parent instead of creating a new root at position 0. The parent already has the system prompt in its KV, so the inner root only needs to prefill the turn separator — **2 tokens instead of ~760**.
...
```
Without warm path: inner root decodes ~760 tokens (full system + tool schemas)
With warm path:    inner root prefills 2 tokens (turn separator only)
