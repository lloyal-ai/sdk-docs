...
After each tool executes, the pool injects `_contextAvailablePercent` into the tool result object before serialization. This is the percentage of KV remaining at that moment:

```json
{"matches": [...], "totalMatches": 4, "_contextAvailablePercent": 32}
```

The value is visible to the agent in its attention window — the model can see how much room remains after each tool call. The same value is emitted on the `agent:tool_result` event for TUI display.
