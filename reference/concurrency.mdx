...
**DISPATCH** executes collected tool calls sequentially. Before each tool, the pool creates a fresh `ContextPressure` snapshot (reflecting KV consumed by SETTLE) and evaluates `policy.shouldExplore(agent, pressure)` to determine explore vs exploit mode. The `explore` flag and `pressurePercentAvailable` are passed to the tool via `ToolContext`. Each tool runs inside `scoped()` + `call()`:
...
Both inner and outer pools create `ContextPressure` snapshots from the same `SessionContext`. Each snapshot captures `nCtx`, `cellsUsed`, `remaining`, and derives `percentAvailable` (0–100 integer). `cellsUsed` is a monotonic counter — it increments on every decode and prefill but does not decrement on individual branch prune (only resets on bulk ops like `retainOnly`). This means `remaining` is a conservative lower bound that becomes increasingly pessimistic as branches are pruned during a run. An inner pool's SETTLE consumes KV that the outer pool's next SETTLE will see as reduced headroom. `percentAvailable` drives `policy.shouldExplore()` — the explore/exploit decision at each DISPATCH. Two thresholds govern behavior:
